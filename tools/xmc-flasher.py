...
def check_python_version():
    ...
    if major != 3:
        raise Exception(
            f"XMC Flasher requires Python3! Current version is {major}.{minor}"
        )
...
def get_jlink_install_path():
    ...
    try:
        key = winreg.OpenKeyEx(
            location, "SOFTWARE\\SEGGER\\J-Link", reserved=0, access=winreg.KEY_READ
        )
    except:
...
def jlink_commander(device, serial_num, cmd_file, console_output=False):
    ...
    try:
        jlink_proc = subprocess.Popen(
            jlink_cmd, stdout=subprocess.PIPE, universal_newlines=True
        )
        if console_output is True:
...
def get_mem_contents(addr, bytes, device, port):
    try:
        ...
        print(e)
    jlink_cmd_file = create_jlink_mem_read_command_file(
        addr, bytes
    )  # todo: comes from proper metafile
...
def check_device(device, port):
    ...
    master_data = read_master_data()
    # get value from reg
    device_value = get_mem_contents(
        master_data[device]["IDCHIP"]["addr"],
        master_data[device]["IDCHIP"]["size"],
        device,
        port,
    )
    device_value_masked = (int("0x" + device_value, 16)) & (
        int("0x" + master_data[device]["IDCHIP"]["mask"], 16)
    )  # take only those bits which are needed
    device_value_masked = f"{device_value_masked:x}"
    device_value_masked = device_value_masked.zfill(
        int(master_data[device]["IDCHIP"]["size"]) * 2
    )
    ...
    if not real_device == device:
        ...
        raise Exception(
            f"Device connected on port {port} does not match the selected device to flash"
        )
...
def check_mem(device, port):
    ...
    if "XMC1" in device:
        ...
        # get value from reg
        device_value = get_mem_contents(
            master_data[device]["FLSIZE"]["addr"],
            master_data[device]["FLSIZE"]["size"],
            device,
            port,
        )
        device_value = int("0x" + device_value, 16) & int(
            "0x0003f000", 16
        )  # bit 17 to bit 12 are needed
        device_value = device_value >> int(
            master_data[device]["FLSIZE"]["bitposition_LSB"]
        )
        ...
        if not flash_size == device.split("-")[1]:
            raise Exception(
                "Memory size of device connected does not match that of the selected device to flash"
            )
        ...
        # get value from reg
        device_value = get_mem_contents(
            master_data[device]["FLASH0_ID"]["addr"],
            master_data[device]["FLASH0_ID"]["size"],
            device,
            port,
        )
        device_value_masked = (int("0x" + device_value, 16)) & (
            int("0x" + master_data[device]["FLASH0_ID"]["mask"], 16)
        )  # take only those bits which are needed
        device_value_masked = f"{device_value_masked:x}"
        device_value_masked = device_value_masked.zfill(
            int(master_data[device]["FLASH0_ID"]["size"]) * 2
        )
    ...
    if serial_num == None or port == None:
        ...
        for port_sn in port_sn_list:
            if port_sn[1] != None:
                real_port = port_sn[0]
                print(
                    f"Device found on port: {real_port}, not on the selected port: {port}."
                )
...
def upload(device, port, binfile, enable_jlink_log):
    ...
    jlink_commander(
        device, serial_num, jlink_cmd_file, console_output=not enable_jlink_log
    )  # console_output = true will store the log to file instead of printing
    remove_jlink_command_file(jlink_cmd_file)
...
def parser():
    ...
    def parser_upload_func(args):
        ...
        if args.port != original_port:
            print(
                f"Please select port {args.port} for using the Serial Monitor or Plotter."
            )
    ...
    parser.add_argument(
        "-v", "--version", action=ver_action, help="xmc-flasher version"
    )
    ...
    required_upload.add_argument(
        "-d", "--device", type=str, help="jlink device name", required=True
    )
    required_upload.add_argument(
        "-p", "--port", type=str, nargs="?", const="", help="serial port"
    )
    required_upload.add_argument(
        "-f", "--binfile", type=str, help="binary file to upload", required=True
    )
    required_upload.add_argument(
    ...
    required_erase.add_argument(
        "-d", "--device", type=str, help="jlink device name", required=True
    )
    required_erase.add_argument(
        "-p", "--port", type=str, help="serial port", required=True
    )
