...
xmc_ino_root_path = os.path.relpath(
...
def build_package(pkg_name):
    ...
    for dir in dirs_to_copy:
        ...
        shutil.copytree(
            os.path.join(xmc_ino_root_path, dir),
            os.path.join(pkg_build_path, dir),
            ignore=ignore_pattern,
        )

    for file in files_to_copy:
        shutil.copyfile(
            os.path.join(xmc_ino_root_path, file), os.path.join(pkg_build_path, file)
        )
...
def get_local_package_index_json():
    with open(
        os.path.join(xmc_ino_root_path, "package/package_infineon_index.template.json"),
        "r",
    ) as f:
        data = json.load(f)
    return data
...
def parser():
    ...
    parser.add_argument(
        "-v", "--version", action=ver_action, help="xmc-release version"
    )
    ...
    parser_release.add_argument(
        ...
        "--root-path",
        type=str,
        default=xmc_ino_root_path,
        help="Path to the XMC-for-Arduino root path",
    )
    parser_release.add_argument(
        "-b",
        "--build-path",
