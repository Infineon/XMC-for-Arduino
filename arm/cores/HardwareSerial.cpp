...
HardwareSerial::HardwareSerial(XMC_UART_t* xmc_uart_config, RingBuffer* rx_buffer, RingBuffer* tx_buffer)
{
...
void HardwareSerial::begin(const uint32_t speed)
{
...
void HardwareSerial::begin( const uint32_t speed, const XMC_UART_MODE_t config )
{
    ...
    XMC_USIC_CH_SetInputSource(_XMC_UART_config->channel, XMC_USIC_CH_INPUT_DX0, _XMC_UART_config->input_source_dx0);

    // Additional input multiplexing
    // Check if dx1 is used
    if (_XMC_UART_config->input_source_dx1 != XMC_INPUT_INVALID)
    {
        XMC_USIC_CH_SetInputSource(_XMC_UART_config->channel, XMC_USIC_CH_INPUT_DX1, _XMC_UART_config->input_source_dx1);
    }
    // Check if dx2 is used
    if (_XMC_UART_config->input_source_dx2 != XMC_INPUT_INVALID)
    {
        XMC_USIC_CH_SetInputSource(_XMC_UART_config->channel, XMC_USIC_CH_INPUT_DX2, _XMC_UART_config->input_source_dx2);
    }
    // Check if dx3 is used
    if (_XMC_UART_config->input_source_dx3 != XMC_INPUT_INVALID)
    {
        XMC_USIC_CH_SetInputSource(_XMC_UART_config->channel, XMC_USIC_CH_INPUT_DX3, _XMC_UART_config->input_source_dx3);
    }
...
void HardwareSerial::IrqHandler( void )
{
    ...
    {
        XMC_UART_CH_ClearStatusFlag(_XMC_UART_config->channel, XMC_UART_CH_STATUS_FLAG_TRANSMIT_BUFFER_INDICATION);
