...
static inline __attribute__((always_inline)) bool directRead(IO_REG_TYPE mask) {
    if (mask > 0x8000)
        return GP16I & 0x01;
    else
...
static inline __attribute__((always_inline)) void directModeInput(IO_REG_TYPE pin) {
    ...
    if (digitalPinIsValid(pin)) {
        ...
        if (rtc_reg) // RTC pins PULL settings
        {
            ...
            ESP_REG(rtc_reg) =
                ESP_REG(rtc_reg) & ~(rtc_gpio_desc[pin].pullup | rtc_gpio_desc[pin].pulldown);
        }
        #endif
        // Input
        if (pin < 32)
...
static inline __attribute__((always_inline)) void directModeOutput(IO_REG_TYPE pin) {
    ...
    #define DIRECT_MODE_INPUT(base, pin) \
        pin_function((PinName)pin, STM_PIN_DATA(STM_MODE_INPUT, GPIO_NOPULL, 0))
    #define DIRECT_MODE_OUTPUT(base, pin) \
        pin_function((PinName)pin, STM_PIN_DATA(STM_MODE_OUTPUT_PP, GPIO_NOPULL, 0))
    ...
    #define DIRECT_MODE_INPUT(base, pin) \
        CY_SYS_PINS_SET_DRIVE_MODE(PORT_ADDRESS(pin) + 8, PIN_IN_PORT(pin), CY_SYS_PINS_DM_DIG_HIZ)
    #define DIRECT_MODE_OUTPUT(base, pin) \
        CY_SYS_PINS_SET_DRIVE_MODE(PORT_ADDRESS(pin) + 8, PIN_IN_PORT(pin), CY_SYS_PINS_DM_STRONG)
...
static inline __attribute__((always_inline)) IO_REG_TYPE directRead(volatile IO_REG_TYPE *base,
                                                                    IO_REG_TYPE pin) {
    IO_REG_TYPE ret;
...
static inline __attribute__((always_inline)) void directModeInput(volatile IO_REG_TYPE *base,
                                                                  IO_REG_TYPE pin) {
    if (SS_GPIO == GPIO_TYPE(pin)) {
...
static inline __attribute__((always_inline)) void directModeOutput(volatile IO_REG_TYPE *base,
                                                                   IO_REG_TYPE pin) {
    if (SS_GPIO == GPIO_TYPE(pin)) {
        WRITE_ARC_REG(READ_ARC_REG(((IO_REG_TYPE)(base) + DIR_OFFSET_SS)) | (0x01 << GPIO_ID(pin)),
                      ((IO_REG_TYPE)(base) + DIR_OFFSET_SS));
...
static inline __attribute__((always_inline)) void directWriteLow(volatile IO_REG_TYPE *base,
                                                                 IO_REG_TYPE pin) {
    if (SS_GPIO == GPIO_TYPE(pin)) {
...
static inline __attribute__((always_inline)) void directWriteHigh(volatile IO_REG_TYPE *base,
                                                                  IO_REG_TYPE pin) {
    if (SS_GPIO == GPIO_TYPE(pin)) {
        WRITE_ARC_REG(READ_ARC_REG(base) | (0x01 << GPIO_ID(pin)), base);
    } else {
...
static inline __attribute__((always_inline)) void directWriteHigh(IO_REG_TYPE mask) {
    ...
    #define noInterrupts()                   \
        osThreadSetPriority(osThreadGetId(), \
                            osPriorityRealtime) // core_util_critical_section_enter()
    #define interrupts() \
        osThreadSetPriority(osThreadGetId(), osPriorityNormal) // core_util_critical_section_exit()
    ...
    #warning \
        "OneWire. RP2040 in Fallback mode. Using API calls for pinMode,digitalRead and digitalWrite."
