...
#if defined(SCU_PARITY_PEEN_PEENDS2_Msk)
...
#endif
#if defined(SCU_PARITY_PEEN_PEENECAT0_Msk)
  XMC_SCU_PARITY_ECAT_MEM = SCU_PARITY_PEEN_PEENECAT0_Msk, /**< ECAT0 SRAM parity error trap. */
#endif
} XMC_SCU_PARITY_t;
...
/**
 *
 * @param memory  The on-chip RAM type, for which the parity logic should be overriden.\n
 *          \b Range: Use type @ref XMC_SCU_PARITY_t to identify the on-chip RAM type. Multiple
 *          memory types can be combined using the \a OR operation.
 *
 * @return None
 *
 * \par<b>Description</b><br>
 * Enable access to parity bits of SRAM modules for various types of peripherals.
 * Enable direct write to parity bits using value selected using XMC_SCU_PARITY_SetParityWriteValue()\n\n
 * \par<b>Related APIs:</b><BR>
 * XMC_SCU_PARITY_SetParityWriteValue() \n\n\n
 *
 */
__STATIC_INLINE void XMC_SCU_PARITY_OverrideParityBitLogic(uint32_t memory)
{
  SCU_PARITY->MCHKCON = memory;
}

/**
 * @param memory  The on-chip RAM type, for which the parity test has to be enabled.\n
 *          \b Range: Use type @ref XMC_SCU_PARITY_t to identify the on-chip RAM type.
 *          \b Only a single memory type should be specified.
 *
 * @return None
 *
 * \par<b>Description</b><br>
 * Selects parity test output from a memory instance that can be inspected using XMC_SCU_PARITY_GetParityReadValue()
 * \par<b>Related APIs:</b><BR>
 * XMC_SCU_PARITY_GetParityReadValue() \n\n\n
 *
 */
__STATIC_INLINE void XMC_SCU_PARITY_SelectMemoryTest(const XMC_SCU_PARITY_t memory)
{
  SCU_PARITY->PMTSR = memory;
}

/**
 * @param write_value parity bits value stored with next write access\n
 *
 * @return None
 *
 * \par<b>Description</b><br>
 * For each byte of a memory module the parity bits corresponding to the next write access are selected here.
 *
 * \par<b>Related APIs:</b><BR>
 * XMC_SCU_PARITY_OverrideParityBitLogic() \n\n\n
 *
 */
__STATIC_INLINE void XMC_SCU_PARITY_SetParityWriteValue(uint8_t write_value)
{
  SCU_PARITY->PMTPR = ((write_value << SCU_PARITY_PMTPR_PWR_Pos) & SCU_PARITY_PMTPR_PWR_Msk);
}

/**
 * @return parity bits generated during last read access
 *
 * \par<b>Description</b><br>
 * For each byte of a memory module the parity bits generated during the most recent read access can be read.
 *
 * \par<b>Related APIs:</b><BR>
 * XMC_SCU_PARITY_OverrideParityBitLogic() \n\n\n
 *
 */
__STATIC_INLINE uint8_t XMC_SCU_PARITY_GetParityReadValue(void)
{
  return ((SCU_PARITY->PMTPR & SCU_PARITY_PMTPR_PRD_Msk) >> SCU_PARITY_PMTPR_PRD_Pos);
}
