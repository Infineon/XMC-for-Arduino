 ...
 * @param dma A constant pointer to XMC_DMA_t, pointing to the GPDMA base address
 * @param channel The destination peripheral request for which DMA channel is to be cleared?
 * @return Total number of data items already read from the source peripheral
 *
 * \par<b>Description: </b><br>
 * Obtain the total number of data items already read from the source peripheral <br>
 *
 * \par<b>Note: </b><br>
 * The width of the single transaction is determined by src_transfer_width.
 *
 */
__STATIC_INLINE uint32_t XMC_DMA_CH_GetTransferredData(XMC_DMA_t *const dma, const uint8_t channel)
{
